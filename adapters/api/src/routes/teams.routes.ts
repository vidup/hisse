...
export function registerTeamsRoutes(
  ...
  app.get<{ Params: { workspaceId: string } }>(
    ...
    async (request) => {
      ...
      return handlers.getTeamsList.execute(
        new GetTeamsListQuery(workspaceId),
      );
    },
  );
  ...
  app.get<{ Params: { id: string } }>(
    ...
    async (request, reply) => {
      ...
      try {
        return await handlers.getTeamWorkflow.execute(
          new GetTeamWorkflowQuery(id),
        );
      } catch (error) {
        const message =
  ...
  app.post<{ Params: { workspaceId: string }; Body: { name: string; description: string; folderPath: string } }>(
    "/api/workspaces/:workspaceId/teams",
    async (request, reply) => {
      const { name, description, folderPath } = request.body;
      try {
        await handlers.createTeam.execute(
          new CreateTeamCommand(name, description, folderPath),
        );
  ...
  app.put<{ Params: { id: string }; Body: { steps: string[] } }>(
    ...
    async (request, reply) => {
      ...
      try {
        await handlers.updateTeamWorkflow.execute(
          new UpdateTeamWorkflowCommand(id, steps),
        );
